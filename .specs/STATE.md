...
- **Feature**: **F1 (video-analysis) — FECHADA** (Verifier PASS, ver abaixo). **F2 (audio-analysis) — Design aprovado pelo usuário, `tasks.md` escrito, aguardando aprovação de Tasks antes de Execute.**
- **Phase / Task**: F2 está na fase **Tasks**. `design.md` foi aprovado pelo usuário e commitado (`4fb87e7`, junto com `pyproject.toml` +3 deps). `.specs/features/audio-analysis/tasks.md` escrito nesta sessão: 13 tarefas atômicas em 5 fases (T1–T3 fundação, T4–T7 raia P1/ICBHI, T8–T10 raia P2/transcrição, T11–T12 raia P3/fadiga, T13 CLI orquestração), Test Coverage Matrix + Gate Commands gerados por amostragem real de `backend/tests/{vitals,video,common}` e `Makefile`, as 3 validações obrigatórias (granularidade, diagrama×dependências, co-localização de teste) todas ✅. 13 tarefas > 8 → empacotamento sugerido em 2 lotes se o usuário aceitar sub-agentes no Execute (Lote 1 = T1–T7, Lote 2 = T8–T13), oferta ainda **não feita** ao usuário (acontece no início do Execute). Nenhum código de pipeline de F2 criado ainda (só `.gitkeep`).
- **Decisões de Design registradas (não re-derivar)**: classificador ICBHI é `RandomForestClassifier(class_weight="balanced")`, 4 classes (`normal/crackle/wheeze/both` — SPEC_DEVIATION documentada no design, ICBHI real tem ciclos com crackle+wheeze simultâneos, 506/6898 medidos nesta sessão); split treino/teste por `GroupShuffleSplit` agrupado por `patient_id` (evita vazamento); subconjunto curado = `icbhi_max_patients` default 40 (de 126 pacientes/6898 ciclos totais, medidos nesta sessão — bate com o "6898" citado na spec); sem persistência de modelo (retreina a cada run, AD-008); `common/config.py` **não é reusado** por F2 (é específico de vitals apesar do nome — F2 cria `pipelines/audio/config.py` próprio, mesmo padrão de validação; flagged como tech debt não-bloqueante em Risks & Concerns do design); `temperature=0.0` fixo no faster-whisper por determinismo (AUDIO-14); léxico de sentimento é lista embutida curada pelo agente (nenhuma lib pt-BR verificada no prazo).
- **Decisão de Tasks registrada (não re-derivar)**: T8 (`transcribe.py`) tem um gap aceito e explícito, documentado em `tasks.md` § "Aberto / Blocker" — o caminho positivo (fala real → transcript correto) só é testável quando o áudio de consulta do grupo existir; a lógica de `reliable` está 100% coberta por unit tests com valores controlados + um teste de integração real usando um áudio ICBHI (som respiratório, sem fala) como caso negativo real, sem mock e sem fabricar áudio de fala. Testes de F2 seguem o padrão dominante do repo (unit com fixtures sintéticas/leves; integration contra dado real já baixado, marcado `@pytest.mark.integration`) — não o padrão isolado de `test_pose.py` (download de modelo sem marcação), porque o modelo faster-whisper é pesado demais para o gate rápido.
- **In-progress**: `.specs/features/audio-analysis/tasks.md` criado; `.specs/features/audio-analysis/spec.md` (status `In Tasks`, coverage 14/14 mapeado) e este `STATE.md` atualizados — **ainda não commitados**, aguardando o usuário aprovar as tarefas antes do commit e do início do Execute.
- **Next step**: Usuário aprova `tasks.md` (ou pede ajuste) → commitar (`tasks.md` + `spec.md` + `STATE.md`) → oferecer empacotamento em sub-agentes (2 lotes) → Execute T1–T13.
- **Nota para a Execute de F2 (não bloqueante para o Design)**: P2/P3 dependem de 1-2 áudios de "consulta" gravados/atuados pelo grupo (spec já assume isso, "confirmed: y", mas o roteiro exato das gravações — "confirmed: n" — ainda não foi definido pelo grupo); nenhum áudio existe em `data/` ainda. Não bloqueia Design/Tasks; só precisa existir antes de rodar P2/P3 de verdade no Execute. ICBHI (P1) já está baixado (`data/icbhi/ICBHI_final_database/`).
- **Verificação de F1 (Verifier independente, 2026-07-22)**: `.specs/features/video-analysis/validation.md`. Resultado: **15/16 ACs com correspondência precisa ao resultado da spec** (VIDEO-01..16); **1 spec-precision gap não bloqueador** (VIDEO-10: sub-cláusula "registrado no CloudWatch" e o cenário nomeado "limite de 1000 detecções" não têm asserção dedicada — o `except Exception` genérico do handler já cobre o comportamento por construção, só falta o teste de observabilidade). Gate: **362 passando, 0 falhas** (292 unit + 70 integração via LocalStack real; `make lint` limpo). Sensor de mutação: **3/3 mortos** (threshold de queda em `pose_detector.py`, limiar de IoU em `object_evaluate.py`, idempotência de nome de evidência em `handler.py`) — nenhum mutante sobreviveu. Lição `L-031` registrada em `.specs/LESSONS.md` (observabilidade: sempre asserir que o log de erro chega ao sink observável, não só que o pipeline segue sem propagar exceção). **Fix opcional não agendado**: teste de integração nomeando o cenário de limite/throttling do Rekognition + confirmação via CloudWatch Logs (Minor, não bloqueador para a demo).
- **Resumo de implementação de F1** (13 tarefas, 2 batches, todas commitadas): T1 `pose_loader`/`models` (`c49be7f`), T2 `pose.py` MediaPipe Task API (`de572fe`), T3 `pose_features` (`7c72a56`), T4 `pose_detector` classify+evidência (`5f30611`), T5 `pose_evaluate` (`43be52f`), T6 `object_loader` COCO real (`c4eeed9`), T7 `object_finetune` YOLOv8n (`0a09db4`), T8 `object_detector` (`a0fe433`), T9 `object_evaluate` (`12aab49`), T10 `adapters.py`/`YoloImageAnalyzer` (`d3d6556`), T11 `handler.py` Lambda fino (`dd0b166`), T12 `infra.py` deploy real (`2c0032c`), T13 `report.py` (`2b07570`). Achados de implementação detalhados: ver histórico git deste arquivo (`git log -p .specs/STATE.md`) se precisar recuperá-los; não repetir a redescoberta.
- **In-progress**: nada em código não commitado. Working tree limpa (`git status` confirmado ao final da verificação).
- **Next step**: Escolher a próxima fatia de trabalho. Candidatos, em ordem de dependência: **F2 (audio-analysis)** — só spec.md existe, precisa de Design (ICBHI + faster-whisper + features acústicas) e Tasks antes do Execute; **F5 (fusion-and-alerting)** — depende das saídas de F1-F4 (AD-024), só faz sentido depois de F2 estar pronta; **frontend** — ainda não iniciado (só README.md), depende de F5 expor a API. Recomendação: seguir a ordem natural e começar F2 (Specify já existe, falta discutir gray areas se houver, Design, Tasks, Execute).
- **Blockers**: none. Rede disponível; Docker funciona direto (grupo `docker` já ativo na sessão atual, sem precisar de `sg docker -c`). **LocalStack foi parado (`make localstack-down`) antes de pausar** — F2 não precisa dele (100% local, sem AWS); rodar `make localstack-up` de novo só quando alguma feature com componente cloud voltar ao Execute.
...
- **Uncommitted files**: `.specs/features/audio-analysis/{spec.md,tasks.md}`, `.specs/STATE.md` (trabalho de Tasks desta sessão, ver Next step acima).
- **Branch**: `feat/f3-vitals-anomaly` (contém F3 + reestruturação + F0 completa + aws-foundation + F4 completa + F1 completa com Verifier PASS; F2 em Design; `main` só tem o commit inicial — estratégia de merge/rename a decidir).
- **Aberto (decisões futuras)**: estratégia de branch/merge para `main`; framework do `frontend/`; DATA-12 (checksum) diferido P3; fix opcional de VIDEO-10 (observabilidade CloudWatch, Minor, não agendado); roteiro exato das gravações de áudio de consulta para F2 P2/P3 (grupo define); escolha exata do classificador respiratório de F2 P1 (Design).
