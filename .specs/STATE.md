...
- **Feature**: Emenda de F0 (URFD/BIDMC) implementada; Verifier em andamento. Próxima construção: aws-foundation (Docker já instalado).
- **Phase / Task**: Docker instalado. Emenda aprovada pelo usuário (manteve Endoscapes, 5 fontes). **T8/T9/T10 implementadas e commitadas**; Verifier independente da emenda rodando em background.
...
- **In-progress**: Verifier independente da emenda F0 (URFD/BIDMC), rodando em background.
- **Next step**: (1) ler o veredito do Verifier da emenda; se FAIL, rotear lacunas (loop de até 3 iterações); se PASS, **F0 fecha por completo** (núcleo + emenda). (2) Rodar `make data` real para baixar URFD (~70 zips) e BIDMC de fato (ainda não baixados no disco — só testados com stubs). (3) Construir **aws-foundation** (spec pronta, Docker disponível): factory boto3 por `ENV`, adapters, IaC idempotente, testes de integração com LocalStack real. (4) **F4** sobre a fundação. F1/F3 ganham as raias/casos novos (URFD pose; BIDMC vitais) quando forem construídas.
- **Blockers**: none. Falta rodar `make data` para baixar URFD/BIDMC de verdade (script testado só com stubs até aqui).
