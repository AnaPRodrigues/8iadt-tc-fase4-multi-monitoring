...
- **Feature**: **F1 (video-analysis) — FECHADA** (Verifier PASS). **F2 (audio-analysis) — FECHADA** (Verifier PASS na iteração 2, ver abaixo). **Emenda em andamento**: camada de treino (`training/`) para produzir pesos reais do YOLOv8 via Colab/GPU — ver ADs a partir de AD-042 e o próprio "Next step" abaixo.
...
- **In-progress (emenda `training/`)**: usuário pediu uma pasta `training/` de topo (irmã de `backend/`/`frontend/`) com notebook Colab para produzir pesos reais do YOLOv8 (F1 hoje só tem fine-tuning smoke/CPU, `epochs=1`, sem split real de validação — `object_finetune.finetune()` usa `val: images/train`, ou seja, valida contra o próprio treino). Investigação feita nesta sessão (não re-verificar): Endoscapes tem `train/`(1212 img)/`val/`(409)/`test/`(312) com `annotation_coco.json` próprio cada, splits oficiais nunca usados por F1; `training/prepare_dataset_subset.py` criado e **testado rodando de verdade** (reusa `object_loader.load_annotated_frames`, produz 1933 imagens/207MB em `training/staging/`, sem tocar nos 36694 jpgs brutos); notebook `training/train_yolo_endoscapes.ipynb` escrito chamando `object_finetune.finetune()` real via `git clone` do repo no Colab (não reimplementa), depois avalia o `best.pt` contra o split `test/` (nunca visto no treino) via `object_loader`+`object_detector.YoloDetector`+`object_evaluate.evaluate` (já existentes em F1) — resolve a tensão entre "F1 não usa split real" e "não pode tocar em F1": o treino em si continua como está (não modificado), a métrica honesta vem de uma avaliação pós-treino separada contra `test/`. **F1 não foi tocada** (nenhum arquivo em `backend/pipelines/video/` ou `backend/tests/video/` modificado). Faltam: `training/requirements-training.txt`, `training/README.md`, `models/README.md`, `.gitignore`/`Makefile` (`models-fetch`), as ADs (AD-042+) formais no `STATE.md`, e apresentar tudo para aprovação do usuário — inclusive a pergunta pendente sobre onde treinar o classificador de áudio (ver Next step).
- **Emenda `training/` — FECHADA**: `training/requirements-training.txt`, `training/README.md`, `models/README.md`, `.gitignore`, `Makefile` (`models-fetch`) escritos e commitados (`a7c5784`); AD-042/AD-043 registradas. Usuário confirma rodar o Colab manualmente por conta própria — nenhuma ação pendente de mim aqui.
- **Documentação sincronizada**: `video-analysis/spec.md` e `prescription-analysis/spec.md` tinham a tabela de rastreabilidade inteira como "Pending" apesar de ambas fechadas (Verifier PASS) — sincronizado a partir dos vereditos reais de cada `validation.md` (commit `214e5d0`). Nenhuma mudança de código.
- **F5 (fusion-and-alerting) — Design ESCRITO, aguardando aprovação do usuário antes de Tasks.** `.specs/features/fusion-and-alerting/design.md` criado nesta sessão. Duas decisões de escopo/arquitetura levantadas e confirmadas pelo usuário, registradas como **AD-044** e **AD-045**: (a) F5 inclui a API fina (`backend/app/`, FastAPI) + Streamlit como framework do `frontend/` (fecha o "a definir" do `frontend/README.md`) — não fica para depois; (b) a linha do tempo cross-modal do paciente-demo é **curada manualmente evento a evento** na config (não derivada automaticamente — os formatos de tempo reais de F1-F4 são incompatíveis entre si, investigado nesta sessão: F3 tem `start_s`, F1-pose só índice de frame, F1-objeto e F2 não expõem tempo, F4 tem timestamp no `evidence_id`); (c) o paciente-demo usa só a raia pose/queda de F1 (URFD), não a raia objeto/Endoscapes (sem relação narrativa com queda/vitais/prescrição). `pyproject.toml` ganhou `fastapi`, `uvicorn`, `httpx`, `requests`, `streamlit` — todos instalados e importam limpo no `.venv` (versões reais: fastapi 0.139.2, uvicorn 0.51.0, httpx 0.28.1, requests 2.34.2, streamlit 1.60.0).
- **Decisões de Design de F5 registradas (não re-derivar)**: `pipelines/fusion/loader.py` lê sidecars JSON reais de `output/<feature>/<run_id>/` (nunca inventa dado); decaimento exponencial `2**(-Δt/half_life_s)`, default 600s; histerese com classe stateful (`HysteresisClassifier`) que só muda de nível ao cruzar limiar±histerese a partir do nível atual (resolve FUSION-13 por construção); severidade por evento é **curada** (default 1.0, overridable), não computada — não há escala comum entre os 4 detectores (confiança 0-1 do áudio vs. z-score não limitado dos vitais), documentado como limitação aceita em Risks & Concerns; dedupe de alerta reusa a MESMA tabela DynamoDB genérica de F4 com prefixo `ALERT#` (não cria tabela nova); nova função aditiva `ensure_subscription` em `aws/provision.py` (aws-foundation, feature fechada) para inscrever e-mails no tópico SNS — mudança puramente aditiva, gate completo deve rodar depois; confirmação de inscrição de e-mail no SNS é manual (clique), documentado como passo operacional, não bug; Streamlit (`frontend/`) só fala HTTP com `backend/app/`, nunca importa `backend/fusion/` direto.
- **Lacuna operacional resolvida**: F1 (raia pose) nunca ganhou um driver ponta a ponta gravando evidência real em `output/video/<run_id>/` (diferente de `vitals/cli.py`/`audio/cli.py`) — sem isso a config curada do paciente-demo não teria `evidence_id` real de F1. Usuário confirmou: F5 ganha uma tarefa nova para `backend/pipelines/video/cli.py` (**arquivo novo, aditivo** — só orquestra `pose_loader`/`pose.py`/`pose_features.py`/`pose_detector.py`/`pose_evaluate.py` já verificados, nenhum modificado; escopo restrito à raia pose, não à raia objeto/Endoscapes, que não entra no paciente-demo por AD-045b). Já adicionado ao `design.md`.
- **In-progress**: `.specs/features/fusion-and-alerting/{spec.md,design.md}`, `.specs/STATE.md`, `pyproject.toml`, `frontend/README.md` — **ainda não commitados**, aguardando o usuário aprovar o design antes do commit e da fase Tasks.
- **Next step**: Usuário aprova `design.md` de F5 (ou pede ajuste) → commitar → fase **Tasks** de F5 (14 requisitos + `pipelines/video/cli.py` + a API/dashboard da AD-044 — bem maior que F1/F2, avaliar quantos batches de sub-agente) → Execute. Execute precisa rodar `pipelines/video/cli.py` (novo) + `make demo` (F3) + o `cli.py` de F2 antes de escrever a config curada do paciente-demo, para ela referenciar `evidence_id`s reais, não fixtures.
...
- **Aberto (decisões futuras)**: estratégia de branch/merge para `main`; framework do `frontend/`; DATA-12 (checksum) diferido P3; fix opcional de VIDEO-10 (observabilidade CloudWatch, Minor, não agendado); roteiro exato das gravações de áudio de consulta para F2 P2/P3 (grupo define); onde treinar o classificador de áudio de F2 (pergunta pendente ao usuário, ver Next step); publicação do primeiro GitHub Release com `best.pt` (bloqueia `make models-fetch` funcionar de verdade); **F5 (fusion-and-alerting)** e **frontend** seguem não iniciados, próximas fatias depois desta emenda.
