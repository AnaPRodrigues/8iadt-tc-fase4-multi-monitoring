...
- **Feature**: **F1 (video-analysis) — FECHADA** (Verifier PASS; nota: `object_finetune.py` foi removida de `backend/pipelines/video/` na emenda AD-046 — treino não é mais responsabilidade do backend, ver abaixo; nenhum outro arquivo de F1 mudou). **F2 (audio-analysis) — FECHADA** (Verifier PASS na iteração 2). **F5 (fusion-and-alerting) — Design aprovado e commitado, aguardando fase Tasks.** Duas emendas transversais concluídas nesta sessão: camada de treino desacoplada (`training/`, AD-042/043) e limpeza de jargão interno fora de `.specs/` (AD-046).
...
- **F5 (fusion-and-alerting) — T1-T18 implementadas nesta sessão (2026-07-23), Verifier FAIL (2 gaps), decisão de fix pendente com o usuário.** `tasks.md` criado (18 tarefas, 6 fases) e executado via 3 lotes de sub-agentes em background, sequenciais: Lote 1 (T1-T8, fundação+motor de fusão, commits `9dd7d1d`..`b44e52b`), Lote 2 (T9-T15, alerta SNS+API, commits `57e122c`..`7019f91`), Lote 3 (T16-T18, dashboard+config curada, commits `22a5808`..`cf613d0`). Gate final: `make test` 487 passando, 0 falhas, `make lint` limpo. Decisão de curadoria confirmada com o usuário durante o Lote 3: caso de vitais do paciente-demo é CTU-UHB (FHR), não BIDMC — BIDMC exigiria implementar um segundo caso inteiro de F3 (loader/features/detectores próprios, nenhum existe hoje), fora de escopo de F5; desencaixe narrativo (monitoramento fetal ao lado de queda de paciente adulto) aceito conscientemente, documentado em `pipelines/fusion/configs/demo.yaml`, consistente com AD-024. O Lote 3 não tinha acesso à ferramenta de sub-agente para despachar um Verifier realmente separado — rodou o "Standalone fallback" documentado em `references/sub-agents.md` (validação fresh-eyes pelo próprio agente que implementou T18, não um sub-agente independente; distinção registrada explicitamente em `validation.md`, não apresentada como equivalente). **Verifier: FAIL** — `.specs/features/fusion-and-alerting/validation.md` (commit `bd273ab`): 16/16 critérios cobertos com evidência `file:line`, 0 spec-precision gaps, mas o sensor de discriminação (4 mutações) achou 2 mutantes sobreviventes (gaps de cobertura de teste em código de produção já correto, não bugs ao vivo, confirmado lendo as linhas mutadas): (1) `pipelines/fusion/hysteresis.py:41` — nenhum teste fixa a fronteira exata verde→vermelho (`threshold_vermelho + hysteresis = 0.75`); (2) `backend/app/routes.py:132` (`_is_confirmed`) — nenhum teste de integração real contra LocalStack cobre o caminho de leitura do DynamoDB (todos os testes de `test_routes.py` fazem monkeypatch da função ou saem antes pela guarda `if not table_name`). Candidatos a lição registrados: `L-036`/`L-037` em `.specs/LESSONS.md`. Ressalva adicional do Lote 3: verificação visual real do dashboard Streamlit (T16/T17) nunca foi feita num navegador (sub-agente sem ferramenta de browser) — só smoke check de processo + `streamlit.testing.v1.AppTest` headless (0 exceções). Também há uma ressalva de que o dataset de vitais usado no paciente-demo (CTU-UHB) é cardiotocografia fetal, não o caso de UTI adulto que combinaria melhor com a queda — aceito por decisão do usuário, ver acima.
...
- **Aberto (decisões futuras)**: estratégia de branch/merge para `main`; framework do `frontend/`; DATA-12 (checksum) diferido P3; fix opcional de VIDEO-10 (observabilidade CloudWatch, Minor, não agendado); roteiro exato das gravações de áudio de consulta para F2 P2/P3 (grupo define); onde treinar o classificador de áudio de F2 (pergunta pendente ao usuário, ver Next step); **F5 (fusion-and-alerting)** — ver bullet dedicado abaixo, T1-T18 implementadas nesta sessão via 3 lotes de sub-agentes, Verifier retornou FAIL (2 gaps de cobertura de teste, não bugs) — decisão de fix pendente com o usuário.
