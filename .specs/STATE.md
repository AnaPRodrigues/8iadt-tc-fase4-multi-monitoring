...
- **Phase / Task**: **F0 FECHADA por completo** (núcleo + emenda). Docker instalado. Próximo: `make data` real (URFD/BIDMC ainda não baixados no disco) e depois aws-foundation.
...
- **In-progress**: nada. Verifier da emenda concluído com PASS (validation.md atualizado, DATA-14/15 Verified, contagem de testes de F0 corrigida para 42).
- **Next step**: (1) Rodar `make data` real para baixar URFD (~70 zips) e BIDMC de fato (ainda não baixados no disco — só testados com stubs até aqui). (2) Construir **aws-foundation** (spec pronta, Docker disponível): factory boto3 por `ENV`, adapters, IaC idempotente, testes de integração com LocalStack real. (3) **F4** sobre a fundação. F1/F3 ganham as raias/casos novos (URFD pose; BIDMC vitais) quando forem construídas.
- **Blockers**: none.
