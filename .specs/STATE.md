...
- **Feature**: **F1 (video-analysis) — FECHADA** (Verifier PASS, ver abaixo). **F2 (audio-analysis) — Design ESCRITO, aguardando confirmação do usuário antes de Tasks.**
- **Phase / Task**: F2 está na fase **Design**, `design.md` escrito nesta sessão (`.specs/features/audio-analysis/design.md`). `pyproject.toml` já ganhou `librosa>=0.11`, `faster-whisper>=1.2`, `praat-parselmouth>=0.4` e `pip install -e .` confirmou as três importam limpo no `.venv` (versões reais: librosa 0.11.0, faster-whisper via `WhisperModel`/`Segment` introspectados, parselmouth 0.4.7/Praat 6.1.38). A receita Praat de jitter/shimmer/HNR foi **testada de verdade** contra um sinal sintético (não só lembrada) — valores plausíveis (jitter≈0.3%, shimmer≈1.8%, HNR≈19.6dB). `spec.md` teve a tabela de rastreabilidade atualizada para `In Design`. Nenhum código de pipeline de F2 criado ainda (só `.gitkeep`).
...
- **In-progress**: `pyproject.toml` modificado (3 deps novas) e `.specs/features/audio-analysis/design.md` criado, `.specs/features/audio-analysis/spec.md` e este `STATE.md` atualizados — **ainda não commitados**, aguardando o usuário revisar/aprovar o design antes do commit e da fase Tasks.
- **Next step**: Usuário confirma o design (ou pede ajuste) → commitar (`pyproject.toml` + `design.md` + `spec.md` + `STATE.md`) → fase **Tasks** de F2 (14 requisitos / 3 stories, avaliar se cabe em 1 batch ~≤8 tarefas ou precisa de 2) → Execute.
...
- **Uncommitted files**: `pyproject.toml`, `.specs/features/audio-analysis/{spec.md,design.md}`, `.specs/STATE.md` (trabalho de Design desta sessão, ver Next step acima).
