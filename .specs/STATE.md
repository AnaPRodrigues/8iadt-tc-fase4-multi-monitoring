...
- **Feature**: **F1 (video-analysis) — FECHADA** (Verifier PASS, ver abaixo). **F2 (audio-analysis) — Design EM ANDAMENTO, pausado a pedido do usuário antes de escrever `design.md`.**
- **Phase / Task**: F2 está na fase **Design** (spec.md já aprovado, sem gray areas — Discuss não foi necessário). Passo interrompido: pesquisa de bibliotecas (Knowledge Verification Chain, Passo 4 — web/PyPI) concluída, `design.md` ainda **não foi escrito**. Nenhum código de F2 criado.
- **Achados de pesquisa de F2 (não re-verificar)**: 3 bibliotecas necessárias confirmadas no PyPI, nenhuma ainda instalada no `.venv` nem em `pyproject.toml`: **librosa** (`0.11.0`, features espectrais/MFCC, AC AUDIO-01), **faster-whisper** (`1.2.1`, transcrição pt-BR, AC AUDIO-06 — já decidido em AD-003), **praat-parselmouth** (`0.4.7`, import `parselmouth`, para jitter/shimmer/HNR, AC AUDIO-11). Checado via `pip index versions <pkg>` nesta sessão (rede disponível). Classificador respiratório (AC AUDIO-02) usa `scikit-learn` (já em `pyproject.toml`), decisão de algoritmo exato (regressão logística vs random forest) ainda em aberto — fica para o Design.
- **In-progress**: nada em código. Nenhum arquivo tocado para F2 ainda (nem `design.md`, nem `pyproject.toml`, nem `backend/pipelines/audio/`). Working tree limpa (`git status` confirmado antes de pausar).
- **Next step**: Retomar o **Design de F2 (audio-analysis)**: (1) adicionar `librosa`, `faster-whisper`, `praat-parselmouth` ao `pyproject.toml`; (2) desenhar a arquitetura das 3 raias (P1 ICBHI/classificador, P2 transcrição/termos/sentimento, P3 fadiga vocal) reaproveitando `common/evidence.py`, `common/metrics.py`, `common/config.py`, `common/logging.py` (mesmo padrão de `pipelines/vitals/`, que também é 100% local — F2 não usa AWS, conforme Out of Scope da spec); (3) escrever `.specs/features/audio-analysis/design.md`. Depois: Tasks (F2 tem 3 stories/14 requisitos, prever se cabe em 1 batch ~≤8 tarefas ou precisa de 2) → Execute.
