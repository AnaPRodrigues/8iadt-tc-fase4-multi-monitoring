...
- **Emenda `training/` — FECHADA nesta sessão**: `training/requirements-training.txt`, `training/README.md`, `models/README.md`, `.gitignore` (padrão `models/*`+`!models/README.md`, igual a `data/`) e `Makefile` (`models-fetch`) escritos; AD-042/AD-043 registradas. Usuário confirmou: (a) resolução do split train/val/test — notebook treina com `finetune()` real de F1 inalterado, avaliação honesta é um passo pós-treino separado contra `test/`, tudo rodando no Colab; (b) classificador de áudio de F2 **mantém-se sem persistência** (código já fechado, Verifier PASS, AD-008 confirmada) — nenhuma AD-044 criada, nenhuma mudança de código em F2. **Nenhum arquivo de F1 ou F2 foi tocado.**
- **Next step**: Commitar `training/` + `models/README.md` + `.gitignore` + `Makefile` + `STATE.md` desta emenda. Depois: escolher a próxima fatia — candidatos são rodar o notebook de treino de verdade (produz o `best.pt` real + primeiro GitHub Release), **F5 (fusion-and-alerting)** (depende de F1-F4, todas prontas exceto — checar F4 —, só falta o Design), ou **frontend** (ainda não iniciado).
