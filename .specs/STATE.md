...
- **Feature**: **aws-foundation FECHADA** (Verifier PASS). Próxima construção: F4 (prescription-analysis), sobre a fundação.
- **Phase / Task**: F0 (núcleo + emenda) e aws-foundation, ambas fechadas com Verifier PASS. LocalStack de pé (`sg docker -c 'docker compose up -d localstack'`) e saudável, usado nos testes de integração reais.
- **Completed**: AD-001 a AD-040. **F3** fechada (Verifier PASS) e migrada para `backend/`. **F0 núcleo** fechado (Verifier PASS). **F0 emenda** (T8 `fetch_urfd`, T9 `fetch_bidmc`, T10 wiring em `main`) implementada: 207 testes verdes (35 de F0), lint limpo. Commits: `acd304f` (T8), `ec54dfa` (T9), `cd42c04` (T10), `8f75c0f` (status). Dois achados não óbvios verificados e corrigidos durante a implementação: (a) `seq -w` do bash ajusta o padding ao valor de N — quebraria `fall-01` para N pequeno (usado nos testes); corrigido com `printf '%02d'` fixo; (b) os "numerics" do BIDMC (HR/PULSE/RESP/SpO2 — o que a AD-040 quer) são um registro **separado** com sufixo `n` que **não está no `RECORDS`** do dataset — `dl_database(records='all')` sozinho não os baixa; `fetch_bidmc` faz uma segunda chamada explícita e só completa com os dois grupos presentes.
- **In-progress**: nada. Verifier de aws-foundation concluído com PASS (validation.md gravado, AWSF-01..07 Verified, contagem de testes corrigida para 47).
- **Completed (aws-foundation)**: `backend/aws/clients.py` (`AwsConfig`/`load_aws_config`/`get_client`), `backend/aws/adapters/__init__.py` (interfaces + registry por `ENV`), `backend/aws/adapters/cloud.py` (`TextractExtractor`/`RekognitionAnalyzer` + `register_cloud_adapters`), `backend/aws/provision.py` (`ensure_bucket`/`ensure_topic`/`ensure_table`/`main`). `make infra-local`/`infra-cloud` e `make localstack-up`/`down` funcionais. Commits T1–T8: `75f421f`,`76f7c44`,`ab8ecf5`,`80d0dfa`,`1e7936b`,`0f2d8a4`,`b303225`,`9d3ce5c`.
- **Next step**: **F4 (prescription-analysis)** — Specify já existe; falta Design + Tasks + Execute, agora consumindo `aws.clients.get_client`/`aws.adapters.get_text_extractor` da fundação (nunca `boto3.client` direto — guarda de T2 pega isso). F4 registra o `TextExtractor` **local** (Tesseract/pdfplumber) para `env=local`, que a fundação deixou em aberto de propósito. F1/F3 ganham as raias/casos novos (URFD pose; BIDMC vitais) quando forem construídas.
- **Blockers**: none.
...
- **Uncommitted files**: nenhum (tudo commitado); `validation.md` da emenda será gravado pelo Verifier.
- **Branch**: `feat/f3-vitals-anomaly` (contém F3 + reestruturação + F0 completa; `main` só tem o commit inicial — estratégia de merge/rename a decidir).
