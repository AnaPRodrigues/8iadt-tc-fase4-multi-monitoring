...
- **Next step**: Escolher a próxima fatia. Candidatos: fase **Tasks** de F5 (14 requisitos + `pipelines/video/cli.py` + API/dashboard — bem maior que F1/F2, avaliar quantos batches de sub-agente); rodar o notebook de treino de verdade no Colab (produz o primeiro `best.pt` real + Release, o usuário faz isso por conta própria). Execute de F5 vai precisar rodar `pipelines/video/cli.py` (novo) + `make demo` (F3) + o `cli.py` de F2 antes de escrever a config curada do paciente-demo, para ela referenciar `evidence_id`s reais, não fixtures.
