...
### AD-054
- **Decision**: O sistema passa a aceitar ficheiros de vídeo reais (.mp4, .avi, .mov, .mkv, .webm) em ambas as raias de vídeo, além dos formatos já suportados (diretório de PNGs URFD e JPEG único Endoscapes). Na raia de pose/movimentação (modalidade `video`), o ficheiro de vídeo tem frames extraídos com `cv2.VideoCapture` (já presente) e é analisado pelo pipeline de pose existente. Na raia cirúrgica (nova modalidade `video_cirurgico`), keyframes são extraídos a cada 2 segundos e analisados pelo YOLOv8 (ou Rekognition conforme `ENV`), com as estruturas críticas encontradas agregadas num resumo clínico único. A separação entre pose e cirurgia é feita por modalidades distintas no frontend/backend (o utilizador escolhe). Zero dependências novas. Três funções novas privadas em `backend/app/analise.py` (`_extrair_frames`, `_analisar_video_pose`, `_analisar_video_cirurgico`), dispatch atualizado em `_analisar_video`, e a nova modalidade `video_cirurgico` adicionada ao dicionário de modalidades, repositório e frontend. Nenhum ficheiro novo criado; ~200 linhas de código adicionadas.
- **Reason**: O enunciado exige "Processar vídeos clínicos (ex: sessões de fisioterapia ou cirurgias gravadas)". Até esta AD, o sistema só aceitava sequências de PNGs (URFD) e JPEGs únicos (Endoscapes) — formatos que não são ficheiros de vídeo. Um .mp4 enviado pelo frontend era silenciosamente mal interpretado como imagem pelo ramo cirúrgico. A correção era necessária para cumprir o requisito literal do enunciado.
- **Trade-off**: A extração de frames em CPU adiciona latência ao upload (um vídeo de 30 s a 30 fps produz ~300 frames extraídos, ~10-30 segundos de processamento adicional). A separação pose/cirurgia exige que o utilizador saiba qual o tipo de vídeo que está a enviar — não há deteção automática de conteúdo. O diretório temporário de frames é limpo automaticamente; a evidência persistente já estava no contrato existente.
- **Scope**: `backend/app/analise.py`, `backend/app/servico.py`, `backend/app/repositorio.py`, `backend/common/atividade.py`, `frontend/src/formatos.js`, `frontend/src/components/AreaEnvio.jsx`, `backend/tests/app/test_analise.py`. Documentado em `.specs/features/video-analysis/{spec,design,tasks,validation}.md` como Amendment 1.
- **Date**: 2026-07-25
- **Status**: active

### AD-055
- **Decision**: A análise de áudio passa a ser um **dispatcher**: se existir `.txt` de anotação ao lado do ficheiro → análise respiratória ICBHI (`_analisar_audio_respiratorio`, comportamento preservado); senão → análise de consulta (`_analisar_audio_consulta`), que compõe transcrição (faster-whisper), features acústicas (jitter, shimmer, HNR), score de fadiga vocal, termos críticos e sentimento — todas funções já existentes em `pipelines/audio/`. O frontend deixa de rotular a modalidade como "Áudio — respiração" e passa a "Áudio" (genérico). A função `_analisar_audio` original foi renomeada para `_analisar_audio_respiratorio` e a nova `_analisar_audio` é o dispatcher. Adicionada a função privada `_analisar_audio_consulta` (~110 linhas) que compõe os pipelines existentes sem os alterar. Nenhum ficheiro novo; nenhuma dependência nova.
- **Reason**: O utilizador reportou que ao fazer upload de um áudio geral (não respiratório), recebia o erro "a análise de áudio espera a anotação de ciclos (arquivo .txt) ao lado da gravação". O pipeline de áudio (F2) já suportava 3 raias (classificação respiratória, transcrição + termos críticos + sentimento, features acústicas + fadiga vocal), mas o dispatch em `_analisar_audio` só expunha a primeira — as outras duas existiam apenas no `cli.py` e nunca foram acessíveis via API/upload. O dispatcher resolve isso sem tocar nos pipelines existentes.
- **Trade-off**: A análise de consulta carrega o faster-whisper em cada execução (sem cache de modelo — mesmo padrão do classificador ICBHI que treina sob demanda). Com um único áudio o baseline de fadiga é degenerado (z-score = 0.0), mesma limitação já documentada no design de F2.
- **Scope**: `backend/app/analise.py` (dispatcher + `_analisar_audio_consulta`), `frontend/src/formatos.js` (rótulo genérico), `backend/tests/app/test_analise.py` (2 testes novos), `.specs/features/audio-analysis/spec.md` (Amendment 1), `.specs/features/video-analysis/design.md` (tabela de modalidades).
- **Date**: 2026-07-25
- **Status**: active

### AD-056
- **Decision**: Refatoração do detector de quedas com três filtros anti-falso-positivo e integração completa do pipeline unificado no upload. (a) **Validação dinâmica via $V_y$**: `validate_fall_dynamic` exige pico de velocidade vertical > 0.10 sustentado em ≥5 frames (média top-5, só valores positivos/descida). Pessoa reclinada desde frame 0 tem $V_y \approx 0$ → classificada como postura estática, sem alerta. (b) **Consistência temporal**: `select_ground_person` exige ≥3 frames consecutivos para validar uma pose — filtra alucinações do MediaPipe em objetos (impressoras, sofás). (c) **YOLO person crop**: stock YOLOv8n (~6MB, COCO) recorta a região da pessoa antes do MediaPipe, reduzindo ruído de fundo. Adicionalmente: evidência cirúrgica agora extrai o keyframe com mais estruturas e desenha bboxes YOLO; `_analisar_video_pose` integrado com pipeline unificado; prescrições avulsas via `make gen-presc`; painel `video_cirurgico` no frontend; áudio como dispatcher geral (com/sem `.txt`).
- **Reason**: Falsos positivos de queda em pessoas sentadas (ex.: `6130119-hd_1920_1080_30fps.mp4`) e alucinações em impressoras (ex.: `cam3.avi`). A análise geométrica estática não distinguia queda de postura reclinada.
- **Trade-off**: YOLO crop adiciona ~50ms/frame em CPU. Limiares de $V_y$ calibrados empiricamente (piso 0.10, 5 frames, limiar 0.15). Stock yolov8n baixado na primeira utilização.
- **Scope**: `pose.py` (YOLO crop), `pose_features.py` (vertical_velocity, temporal filter), `pose_detector.py` (validate_fall_dynamic), `analise.py` (unified pipeline, surgical evidence), `adapters.py` (bboxes), `generator.py` (standalone CLI, receipt layout), `PacienteDetalhe.jsx` (video_cirurgico panel), `formatos.js` (áudio genérico).
- **Date**: 2026-07-25
- **Status**: active

| Requisito do enunciado | Feature / cobertura | Status |
| --- | --- | --- |
| Req.1 — Vídeo: análise postural (OpenPose/pose) | F1 raia pose: URFD + MediaPipe Pose (AD-039) **+ suporte a ficheiros .mp4/.avi/.mov/.mkv/.webm via upload (AD-054)** — **FECHADO, Verifier PASS** | ✅ Coberto |
| Req.1 — Vídeo: detecção de objeto/área crítica (YOLOv8) | F1 raia objeto: Endoscapes/Cholec80 + YOLOv8/Rekognition (AD-033/035) **+ suporte a vídeo cirúrgico via modalidade `video_cirurgico` com keyframes a cada 2 s (AD-054)** — **FECHADO, Verifier PASS** | ✅ Coberto |
...
- **Varredura de jargão em `pipelines/fusion/` — FECHADA nesta sessão**, a pedido do usuário. `backend/pipelines/fusion/` (módulo de F5, construído *depois* da varredura da AD-046) ainda tinha vocabulário interno fora de `.specs/` — códigos `FUSION-NN`, `F1`/`F5`, `AD-045a` em docstrings/comentários de `config.py`, `hysteresis.py`, `models.py`, `loader.py`, `risk_engine.py`, `transitions.py`, `configs/demo.yaml` e nos respectivos testes (`backend/tests/fusion/test_{alert,hysteresis,risk_engine,fusion_loader,transitions}.py`). Removido, preservando 100% do raciocínio técnico (mesmo princípio da AD-046) — mudança só de comentário/docstring, nenhuma linha de código executável tocada. De passagem, também corrigido um comentário de `models.py` que ainda citava `link_s3_evidencia` num campo que hoje aponta para `/evidence/{id}` da API (S3 não existe mais desde AD-048; `alert.py` já estava correto, só o comentário do dataclass é que não tinha acompanhado). Verificação: `make test` sem regressão (420 unit + 55 integration + 3 skipped = 478, mesma contagem de antes), `ruff check backend` limpo, grep de jargão (`F[0-5]`, `AD-NNN`, `FUSION-NN` etc.) em `pipelines/fusion/`/`tests/fusion/` retorna vazio.
- **F1-Pose expansion (Video Pose Analysis) — FECHADA (2026-07-25).** 10 tarefas spec-driven (`a18a2d0`…`b199265`). Pipeline unificado: queda com persistência + fisioterapia + multi-pessoa + evidência anotada. Verifier PASS.
- **Refatoração anti-falso-positivo (2026-07-25) — FECHADA.** Commits `6cf817e`…`c13a631`. Três filtros: (a) validação dinâmica via $V_y$ (exige pico de velocidade vertical sustentado, elimina falsos positivos em pessoas reclinadas), (b) consistência temporal (≥3 frames consecutivos para validar pose, filtra alucinações em objetos), (c) YOLO person crop (stock YOLOv8n recorta pessoa antes do MediaPipe). Evidência cirúrgica: keyframe com mais estruturas + bboxes YOLO desenhadas. `_analisar_video_pose` integrado com pipeline unificado.
