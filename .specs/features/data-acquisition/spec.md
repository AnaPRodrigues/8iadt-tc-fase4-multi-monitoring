...
- [ ] Baixar as três fontes abertas sem barreira (CTU-UHB, ICBHI 2017, Cholec80-CVS) por um único comando (`make data`).
...
| Cholec80 original (formulário CAMMA) | AD-016: substituído pelo Cholec80-CVS aberto |
...
| Download de todo o Cholec80-CVS | Só os poucos vídeos usados na demo (via seleção de LFS) — o dataset completo é grande demais para o escopo |
...
| `git`, `wget`, `unzip`, `git-lfs` disponíveis | Assumidos presentes; script verifica e falha com instrução se faltar | Ambiente é notebook Linux; dependências de sistema comuns, mas a ausência precisa de mensagem clara | y |

**Open questions:** duas confirmações bloqueiam a EXECUÇÃO (não a spec): URL do ICBHI e repo do Cholec80-CVS. Serão perguntadas uma a uma antes de rodar o script. Nada é assumido silenciosamente.
...
| External-dependency failure | Rede, ferramenta ausente (`wget`/`git`/`unzip`/`git-lfs`) ou URL inválida param aquele dataset com erro claro, sem deixar `.complete` — ver DATA-09, DATA-11 |
...
Cholec80-CVS com um único comando idempotente, para reproduzir o ambiente de dados sem passos
manuais.
...
3. WHEN o script roda THEN o sistema SHALL obter o Cholec80-CVS em `data/cholec80-cvs/` via `git clone`, buscando por Git LFS apenas os vídeos usados na demo.
...
3. WHEN uma ferramenta necessária (`wget`, `git`, `unzip`, `git-lfs`, `python`/`wfdb`) está ausente THEN o sistema SHALL falhar com mensagem que nomeia a ferramenta e como instalá-la, sem deixar sentinela `.complete`.
...
- WHEN o Cholec80-CVS usa Git LFS mas `git-lfs` não está instalado THEN o sistema SHALL falhar com mensagem específica, sem deixar um clone sem os vídeos como se estivesse completo.
...
| DATA-03 | P1: Clone Cholec80-CVS (LFS seletivo) | Design | Pending |
...
- [ ] `make data` popula `data/ctu-uhb/`, `data/icbhi/` e `data/cholec80-cvs/` a partir do zero, sem passo manual.
